...
async def faq_premium(callback: CallbackQuery):
    ...
    text = (
        "💳Подписка предоставляем Вам ежемесячно определенное кол-во токенов, в зависимости от выбранного тарифа. "
...
async def faq_tokens(callback: CallbackQuery):
    ...
    text = (
        "💎 Токены это виртуальная валюта с помощью которой Вы можете оплачивать Ваши генерации в нейросетях и "
...
async def faq_recommendations(callback: CallbackQuery, callback_data: FAQCallback):
    ...
    if sub_chapter == FAQRecButtons.USE.name:
        text = (
            "🔹 Телеграм  бот предоставляет доступ к нейросетям для генерации текст, изображений, и даже "
        ...
        text = (
            "🔹 Чтобы получить нужный результат от работы нейросети, нужно научиться правильно составлять запросы. "
        ...
        text = (
            "🔹 Мы создали  сервис для полноценной генерации учебных работ: дипломов, курсовых, эссе, рефератов. "
...
async def faq_problems(callback: CallbackQuery, callback_data: FAQCallback):
    ...
    if sub_chapter == FAQProblemsButtons.AI.name:
        text = (
            "🔹 Нейросети могут не работать по разным причинам. Если что-то не работает, нам отправляется "
        ...
        text = (
            "🔹 Сервисы могут не работать по разным причинам. Если что-то не работает, нам отправляется "
...
async def faq_inline(callback: CallbackQuery, callback_data: FAQCallback):
    ...
    if sub_chapter == "cmds":
        ...
        text = (
            "🔹Наш телеграмм бот можно добавить в Ваш чат телеграмм, и тогда все участники смогут им пользоваться, "
...
async def faq_finances(callback: CallbackQuery, callback_data: FAQCallback):
    ...
    if sub_chapter in (FAQFinancesButtons.PREMIUM.name, FAQFinancesButtons.TOKENS.name):
        text = (
            "🔹Время работы оператора: с 10:00 до 20:00 с понедельника по пятницу. Пожалуйста, приготовьте чек, "
        ...
        text = (
            "🔹Продление выбранной подписки происходит автоматически, но ее можно отключить в разделе профиль. "
        ...
        text = (
            f"🔹При оформлении подписки Вы согласились с "
        ...
        text = (
            "🔹У нас есть ответы на часто задаваемые вопросы касательно финансов. Ознакомьтесь с ними и следуйте "
