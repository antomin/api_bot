...
async def change_balance(user: User, model: Model, add: bool = False) -> None:  # TODO Review
    ...
    if user.tariff:
        if model.name in ("ChatGPT 3.5 Turbo", "Kandinsky", "Gemini"):
            return
...
async def create_report(auto: bool = False) -> Report:
    ...
    async with db.async_session_factory() as session:
        ...
        users_with_link_cnt = await session.scalar(select(func.count(User.id)).where(exists().where(User.referal_links)))
        new_users_cnt = await session.scalar(select(func.count()).select_from(User).where(User.created_at.between(*time_range)))
        new_users_with_link_cnt = await session.scalar(select(func.count(User.id)).where(exists().where(User.referal_links), User.created_at.between(*time_range)))
